...
-------------------------------------------*/
int calendar = date.internal();  
 float calc = calendar + serialEvent;
  /= ; 
 
date.internal();
int s, a, h, d, m, y;

/* Обьединить переменные s, a для оптимизации;
--------------------БАГА------------------------*/
if (s =0; s >= 60)
{
 int (a+1); 
...
/* ВАЖНО!
вход значения даты для внтреннего счетчика даты с RS232 в обратном порядке year, month, day, hour, minute, second.*/
  ...
  void serialEvent(Serial myPort)
{
 ...
 работает,но требуется лучший алгоритм конвертирования.*/
 --------------------------------------------------------------
 int input = analogRead(sensorPin); //значение с аналогово pin -a.
 float voltage = input * 5.0; //float переменная с плаваюшей запятой.
 voltage /= 1024.0; 
